...
// ============================================================
//  TITAN PROFILE — École du Tigre-inspired brutal design
// ============================================================
...
export default function ProfileClient({
  ...
  const handleFollow = async () => {
    ...
    if (following) {
      setFollowing(false); setFollowersCount(c => c - 1)
      await supabase.from('follows').delete().eq('follower_id', currentUserId).eq('following_id', profile.id)
      toast.success('Unfollowed')
    } else {
      setFollowing(true); setFollowersCount(c => c + 1)
    ...
    <div style={{ background: G.black, minHeight: '100vh', fontFamily: "'Barlow Condensed', sans-serif" }}>
      ...
      <div style={{ position: 'relative', height: 240, overflow: 'hidden' }}>
        {(profile as any).banner_url ? (
          <img src={(profile as any).banner_url} alt="banner"
      ...
      <div style={{ padding: '0 16px', marginTop: -60, position: 'relative', zIndex: 3 }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:16 }}>
          <div style={{ position:'relative' }}>
            <div style={{ width:88, height:88, borderRadius:4, padding:3, background:`linear-gradient(135deg, ${G.gold}, ${G.goldDim})`, boxShadow:`0 0 30px ${G.gold}44` }}>
              <img src={profile.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${profile.username}`}
                alt="avatar" style={{ width:'100%', height:'100%', borderRadius:2, objectFit:'cover', filter:'grayscale(20%)' }} />
            </div>
            {streak > 0 && (
        ...
        <div style={{ marginBottom:20 }}>
          ...
          <div style={{ display:'flex', gap:16, marginBottom:10 }}>
            <span style={{ fontSize:13 }}><strong style={{ color:G.gold }}>{followersCount}</strong> <span style={{ color:G.grey }}>followers</span></span>
            <span style={{ fontSize:13 }}><strong style={{ color:G.gold }}>{followingCount}</strong> <span style={{ color:G.grey }}>following</span></span>
          </div>
        ...
        {(displayPRs.length > 0 || isOwn) && (
          <div style={{ marginBottom:28 }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:12 }}>
              ...
              {isOwn && (
                <Link href={`/${profile.username}/prs`} style={{
                  fontSize:10, color:G.gold, textDecoration:'none', fontWeight:700,
                  letterSpacing:'0.1em', textTransform:'uppercase',
                  border:`1px solid ${G.gold}44`, padding:'4px 12px',
                }}>EDIT →</Link>
              )}
            </div>
                ...
                <div style={{ display:'grid', gridTemplateColumns: displayPRs.length < 3 ? `repeat(${displayPRs.length}, 1fr)` : '1fr 1fr 1fr', gap:6 }}>
                  {displayPRs.map(pr => (
                    <div key={pr.id} style={{
                      ...
                      borderTop:`3px solid ${pr.validated ? G.gold : G.border}`,
                      padding:'14px 10px', textAlign:'center', position:'relative', overflow:'hidden',
                    }}>
                      <div style={{ position:'absolute', bottom:-6, right:-2, fontSize:52, fontWeight:900, color:G.gold, opacity:0.04, lineHeight:1, pointerEvents:'none' }}>
                        {pr.weight}
                      </div>
        ...
        {(palmares.length > 0 || isOwn) && (
          <div style={{ marginBottom:28 }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:12 }}>
              ...
              <div style={{ flex:1, height:1, background:G.border }} />
              {isOwn && (
                <Link href={`/${profile.username}/palmares`} style={{
                  fontSize:10, color:G.gold, textDecoration:'none', fontWeight:700,
                  letterSpacing:'0.1em', textTransform:'uppercase',
                  border:`1px solid ${G.gold}44`, padding:'4px 12px',
                }}>EDIT →</Link>
